	...
	public void salvar(ValidacaoBoletoController ctrl) throws Exception {
	    ...
	    if (isTodosValidados(ctrl)) {
	        ...
	        if(isRequerimentoInexigibilidade(requerimento)) {
	            ...
	            // Usa o método que já existe ProcessoService
	            if(!processoService.hasProcesso(requerimento.getIdeRequerimento())) {
	                try {
	                    requerimentoService.finalizar(requerimento, getOperador(), false);
	                } catch (Exception e) {
	                    String errorMsg = e.getMessage() != null ? e.getMessage().toLowerCase() : "";
	                    boolean isConstraintError = e instanceof java.sql.BatchUpdateException ||
	                        e instanceof org.hibernate.exception.ConstraintViolationException ||
	                        (e instanceof javax.persistence.PersistenceException && e.getCause() instanceof org.hibernate.exception.ConstraintViolationException) ||
	                        errorMsg.contains("constraint") || errorMsg.contains("duplicate") || errorMsg.contains("já existe") || 
	                        errorMsg.contains("entrada em lote") || errorMsg.contains("foi abortada");
	        ...
	        if(ctrl.isDirucSecundaria()) {
	            requerimentoService.duplicarControleTramitacaoParaAreaSecundaria(requerimento, getOperador(), true);
	        }
	    } else {
	    ...
	    if (!Util.isNull(requerimento.getIdeArea())) {
	        requerimentoService.atualizarArea(requerimento);
	    }
