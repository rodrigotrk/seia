...
public class NovoRequerimentoServiceFacade {
	...
	public void salvarRequerimentoAposEndereco(ActionEvent event, NovoRequerimentoController novoRequerimentoController,
	        IdentificarPapelController identificarPapelController) {
	    ...
	    try {
	        if (novoRequerimentoController.isRequerentePJ()) {
	            identificarPapelController.setPessoaJuridica(
	                    ContextoUtil.getContexto().getReqPapeisDTO().getRequerente().getPessoa().getPessoaJuridica());
	        }

	        if (novoRequerimentoController.isRequerentePF()
	                || (novoRequerimentoController.isRequerentePJ() && identificarPapelController.validarPJ())) {
	            ...
	            if (!Util.isNullOuVazio(empreendimento.getIdeEmpreendimento())) {
	                ...
	                boolean requerimentoAberto = novoRequerimentoService
	                        .verificaRequerimentoEmpreendimentoExistente(empreendimento);

	                if (requerimentoAberto) {
	                    novoRequerimentoController.setDesabilitarTudo(false);
	                    novoRequerimentoController.setMostraQuestionarioAposSalvar(false);
	                    JsfUtil.addWarnMessage(
	                            "Não será possível abrir um novo requerimento para este empreendimento, pois o mesmo já apresenta um requerimento em aberto aguardando a formação do processo.");
	                    return;
	                }
	                ...
	                if (temp != null) {
	                    if (!tramitacaoRequerimentoService
	                            .buscarUltimaTramitacaoPorRequerimento(temp.getIdeRequerimento())
	                            .getIdeStatusRequerimento().getIdeStatusRequerimento()
	                        ...
	                        if (!Util.isNullOuVazio(temp) && Util.isNullOuVazio(temp.getNumRequerimento())) {
	                            ...
	                            novoRequerimentoController.getRequerimentoSelecionado()
	                                    .setIdeRequerimento(requerimento.getIdeRequerimento());
	                ...
	                if (!Util.isNullOuVazio(empreendimento.getIdeEmpreendimento())) {
	                    requerimento
	                            .setEmpreendimentoRequerimentoCollection(new ArrayList<EmpreendimentoRequerimento>());
	                ...
	                // Popula as tabelas de tramitacao e pessoa
	                ...
	                // Verifica se os registros foram populados.
	                if (requerimento.getTramitacaoRequerimentoCollection() == null
	                        || requerimento.getTramitacaoRequerimentoCollection().isEmpty()) {
	                    throw new RuntimeException("Falha na criação do registro da tramitação do requerimento.");
	                }

	                if (requerimento.getRequerimentoPessoaCollection() == null
	                        || requerimento.getRequerimentoPessoaCollection().isEmpty()) {
	                    throw new RuntimeException("Falha na criação do registro de requerimento pessoa.");
	                }
	                ...
	                if (!Util.isNullOuVazio(empreendimento)) {
	                    if (!Util.isNullOuVazio(requerimento)
	                        ...
	                        // Garante que as relações estão corretamente configuradas antes de atualizar
	                        estabelecerVinculosEntreEntidades(requerimento);
	                        
	                        requerimentoUnicoService.atualizarRequerimento(requerimento);
	                        JsfUtil.addSuccessMessage("Requerimento atualizado com sucesso.");
	                    } else {
	                        // Garante que as relações estão corretamente configuradas antes de inserir
	                        estabelecerVinculosEntreEntidades(requerimento);
	                        
	                        // Verifica se realmente temos uma tramitação e um requerimentoPessoa antes de persistir
	                        if (validarAssociacoesObrigatorias(requerimento)) {
	                            ...
	                            perguntaRequerimentoService
	                                    .salvarOuAtualizarPerguntaReq(novoRequerimentoController.getPergunta0_1());
	                            
	                            JsfUtil.addSuccessMessage("Requerimento salvo com sucesso.");
	                        } else {
	                            JsfUtil.addErrorMessage("Não foi possível salvar o requerimento: faltam associações obrigatórias.");
	                            return;
	                        }
	                    }
	                } else {
	...
	/**
	 * Configura corretamente os vinculos entre o requerimento e suas coleções
	 */
	private void estabelecerVinculosEntreEntidades(Requerimento requerimento) {
	    // Configura corretamente a relação requerimento com TramitacaoRequerimento
	    if (requerimento.getTramitacaoRequerimentoCollection() != null) {
	        for (TramitacaoRequerimento tramitacao : requerimento.getTramitacaoRequerimentoCollection()) {
	            tramitacao.setIdeRequerimento(requerimento);
	        }
	    }
	    
	    // Configura corretamente a relação requerimento com RequerimentoPessoa
	    if (requerimento.getRequerimentoPessoaCollection() != null) {
	        for (RequerimentoPessoa reqPessoa : requerimento.getRequerimentoPessoaCollection()) {
	            reqPessoa.setRequerimento(requerimento);
	        }
	    }
	    
	    // Configura corretamente a relação requerimento com EmpreendimentoRequerimento
	    if (requerimento.getEmpreendimentoRequerimentoCollection() != null) {
	        for (EmpreendimentoRequerimento empReq : requerimento.getEmpreendimentoRequerimentoCollection()) {
	            empReq.setIdeRequerimento(requerimento);
	        }
	    }
	}

	/**
	 * Valida se o requerimento possui as associações obrigatórias
	 */
	private boolean validarAssociacoesObrigatorias(Requerimento requerimento) {
	    // Verifica se temos ao menos uma tramitação.
	    if (requerimento.getTramitacaoRequerimentoCollection() == null 
	            || requerimento.getTramitacaoRequerimentoCollection().isEmpty()) {
	        JsfUtil.addErrorMessage("O requerimento deve possuir pelo menos uma tramitação.");
	        return false;
	    }
	    
	    // Verifica se temos ao menos uma pessoa associada.
	    if (requerimento.getRequerimentoPessoaCollection() == null 
	            || requerimento.getRequerimentoPessoaCollection().isEmpty()) {
	        JsfUtil.addErrorMessage("O requerimento deve possuir pelo menos uma pessoa associada.");
	        return false;
	    }
	...
	/**
	 * Método existente adaptado para garantir que a tramitação está corretamente vinculada ao requerimento.
	 */
	private void adicionarTramitacao(StatusRequerimentoEnum status, Requerimento requerimento, Pessoa pessoa) {
	    ...
	    tramitacaoRequerimento.setIdeRequerimento(requerimento); // Adiciona referência ao requerimento.
	    tramitacaoRequerimento.setDtcMovimentacao(new Date()); // Adiciona data da tramitação.
