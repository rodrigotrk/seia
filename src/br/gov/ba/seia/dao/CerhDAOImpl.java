...
public class CerhDAOImpl extends AbstractDAO<Cerh> {
	...
	public Collection<Cerh> listarCerh(Empreendimento empreendimento) {
		...
		DetachedCriteria criteria = DetachedCriteria.forClass(Cerh.class, "c")
			...
			.add(Restrictions.eq("c.indExcluido", false))
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	private DetachedCriteria getCriteriaListarCerh(Map<String, Object> params) throws Exception {
	...
	private void adicionarRestrictions(DetachedCriteria criteria, Map<String, Object> params) throws Exception {

		criteria
			.add(Restrictions.eq("te.ideTipoEndereco", TipoEnderecoEnum.LOCALIZACAO.getId()))
			.add(Restrictions.eq("c.indHistorico", false))
			.add(Restrictions.or(Restrictions.eq("c.indExcluido", false),Restrictions.isNull("c.indExcluido")))
			//.add(Restrictions.ne("c.indExcluido", false))
		;
		...
		usuarioConvenio(criteria);

	}

	private void usuarioConvenio(DetachedCriteria criteria) throws Exception{
		if(ContextoUtil.getContexto().getUsuarioLogado().isUsuarioExterno()) {
			...
			List<ContratoConvenio> convenios = contratoConvenioDAOImpl.listarConveniosPorPessoa(idePessoaFisica);
			...
			Junction clausulasEspeciais = Restrictions.disjunction()
				.add(Restrictions.eq("r.idePessoa", idePessoaFisica))
				.add(Restrictions.eq("ppf.ideProcurador.idePessoaFisica", idePessoaFisica))
				.add(Restrictions.eq("rpj.idePessoaFisica.idePessoaFisica", idePessoaFisica))
				.add(Restrictions.eq("ppj.ideProcurador.idePessoaFisica", idePessoaFisica));

			if(convenios !=null && !convenios.isEmpty()){
				clausulasEspeciais.add(Restrictions.in("ideContratoConvenio", convenios));
			}

			criteria.add(clausulasEspeciais);
		}
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public Cerh getNumeroAtualCerh(Cerh cerh) {
		DetachedCriteria criteria = DetachedCriteria.forClass(Cerh.class)
			.add(Restrictions.eq("ideCerh", cerh.getIdeCerhPai().getId()))
			.setProjection(Projections.projectionList()
					.add(Projections.property("numCadastro"),"numCadastro")
			).setResultTransformer(new AliasToNestedBeanResultTransformer(Cerh.class));

		return cerhDAO.buscarPorCriteria(criteria);
	}
}
