...
public class EmpreendimentoService implements Serializable{
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void salvarEmpreendimento(Empreendimento pEmpreendimento)  {
		if (!Util.isNullOuVazio(pEmpreendimento.getImovelCollection())) {
			for (Imovel lImovel : pEmpreendimento.getImovelCollection()) {
				ImovelUrbano lImovelUrbano = !Util.isNullOuVazio(lImovel.getImovelUrbano()) ? lImovel.getImovelUrbano().getClone() : null;
				lImovel.setImovelUrbano(null);
				imovelService.salvarAtualizarImovel(lImovel);
				if(!pEmpreendimento.getIndCessionario()) {
	...
	public void atualizarEmpreendimento(Empreendimento pEmpreendimento)  {
		if (!Util.isNullOuVazio(pEmpreendimento.getImovelCollection())) {
			for (Imovel lImovel : pEmpreendimento.getImovelCollection()) {
				if (!Util.isNullOuVazio(lImovel.getImovelUrbano()) && !Util.isNullOuVazio(lImovel.getImovelUrbano().getNumInscricaoIptu())) {
					lImovel.getImovelUrbano().setIdeImovelUrbano(lImovel.getIdeImovel());
					imovelUrbanoService.salvarAtualizarImovelUrbano(lImovel.getImovelUrbano());
				} else if(Util.isLazyInitExcepOuNull(lImovel.getIdeTipoImovel())){
					if(imovelService.retornaTipoImovelPorIdeImovel(lImovel.getIdeImovel()).equals(3)){
						lImovel.setIdeTipoImovel(new TipoImovel(3));
						imovelService.salvarAtualizarImovel(lImovel);
					}
				} else if(lImovel.getIdeTipoImovel().equals(new TipoImovel(3))){
					imovelService.salvarAtualizarImovel(lImovel);
				}
			}
		}
