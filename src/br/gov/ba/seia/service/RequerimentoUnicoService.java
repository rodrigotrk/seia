...
import br.gov.ba.seia.dao.RequerimentoDAOImpl;
...
public class RequerimentoUnicoService {
	...
	@Inject
	private RequerimentoDAOImpl requerimentoDAOImpl;
	@Inject
	...
	public String gerarNumero(Requerimento requerimento) throws Exception {
		...
		while(primeiraConsulta || requerimentoService.isExisteNumRequerimento(requerimento)){	
			...
			numeroRequerimento 
				.append(getNumByOrgaoByAno(requerimento, contador,"REQ"))
				.append('/');
	...
	public String gerarNumeroTipo(Requerimento requerimento, String tipo) throws Exception {
		...
		while(primeiraConsulta || requerimentoService.isExisteNumRequerimento(requerimento)){	
			...
			numeroRequerimento 
				.append(getNumByOrgaoByAno(requerimento, contador,tipo))
				.append('/');
	...
	private String getNumByOrgaoByAno(Requerimento requerimento, int incremento,String tipo) throws Exception, NumberFormatException {
	    
	    // Obtém o último número de requerimento a partir do banco
	    Requerimento ultimoRequerimento = requerimentoDAOImpl.getMaiorNumeroRequerimentoByOrgaoByAnoAtual(requerimento,tipo);
	    String ultimoNumRequerimento = (ultimoRequerimento != null) ? ultimoRequerimento.getNumRequerimento() : null;
	     
	    String novoSequencial;
	    if (ultimoNumRequerimento != null && ultimoNumRequerimento.length() >= 16) {
	    	
	    	
	        int indiceUltimoPonto = ultimoNumRequerimento.lastIndexOf(".");
	        int indiceBarra = ultimoNumRequerimento.indexOf("/", indiceUltimoPonto);
	        
	        String seqStr = ultimoNumRequerimento.substring(indiceUltimoPonto + 1, indiceBarra);
	        
	        seqStr = Util.rpad(seqStr, 6, '0');
	        
	        int seq = Integer.parseInt(seqStr);
	        
	        seq += incremento;  // Incrementa o valor da sequência
	        
	        novoSequencial = Util.lpad(Integer.toString(seq), '0', 6);
	    } else {
	        novoSequencial = Util.lpad(Integer.toString(incremento), '0', 6);
	    }
...
//				.add(Restrictions.eq("indAssinaturaObrigatoria", Boolean.TRUE))
	...
	public String gerarNumeroDTRP(Requerimento requerimento) throws Exception {
		...
		while(primeiraConsulta || requerimentoService.isExisteNumRequerimento(requerimento)){	
			...
			numeroRequerimento 
				.append(getNumByOrgaoByAno(requerimento, contador,"DTRP"))
				.append('/');
