...
public class DeclaracaoInexigibilidadeService {
	...
	private String getNumByOrgaoByAno(Requerimento requerimento, int contador) throws Exception {
		Requerimento ultimoRequerimento = requerimentoDAOImpl.getMaiorNumeroRequerimentoByOrgaoByAnoAtual(requerimento,"INEXIG");
		...
		if (StringUtils.isNotBlank(ultimoNumRequerimento) ) {
			if (ultimoNumRequerimento.length() >= 16) {
				 ...
				 int indiceUltimoPonto = ultimoNumRequerimento.lastIndexOf(".");
			        ...
			        seq += contador;  // Incrementa o valor da sequência
				
				numRequerimento = Util.lpad(Integer.toString(seq), '0', 6);
			}
		}else{
