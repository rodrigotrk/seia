...
public class LoteBoletoService {
	...
	private ByteArrayOutputStream getRemessa(LoteBoleto lote, List<BoletoPagamentoRequerimento> boletos, LoteRemessaBoleto remessa) throws IOException {
		...
		for(BoletoPagamentoRequerimento b : boletos){
			...
			Pessoa requerente = pessoaDAOImpl.carregarDadosRequerente(ideRequerimento, null);
			
			writer.write(getSegmentoP(dadoBancario, b, qtdDetalhes)); qtdDetalhes++;
			writer.write(getSegmentoQ(dadoBancario, b, qtdDetalhes, requerente)); qtdDetalhes++;
		...
		writer.write(getTrailerLote(boletos.size()*2+2)); //qtd de segmentos (P e Q) mais 2 (header e trailer de lote)
		writer.write(getTrailerArquivo(boletos.size()*2+4)); //qtd de segmentos (P e Q) mais 4 (header e trailer de lote; header e trailer de arquivo)
		writer.close();
