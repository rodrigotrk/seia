...
public class EnquadramentoService {
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public Enquadramento buscarEnquadramentoPorRequerimentoAPE(Enquadramento enquadramento) {
	    DetachedCriteria criteria = DetachedCriteria.forClass(Enquadramento.class, "e")
	            .createAlias("ideRequerimento", "r", JoinType.INNER_JOIN)
	            .add(Restrictions.eq("r.ideRequerimento", enquadramento.getIdeRequerimento().getIdeRequerimento()))
	    		.add(Restrictions.eq("r.indExcluido", false));
	    List<Enquadramento> listaEnquadramentos = enquadramentoDAO.listarPorCriteria(criteria);
	    Enquadramento enquadramentoTemp = null;
	    if (!Util.isNullOuVazio(listaEnquadramentos) && !listaEnquadramentos.isEmpty()) {
	        enquadramentoTemp = listaEnquadramentos.get(listaEnquadramentos.size() - 1);
