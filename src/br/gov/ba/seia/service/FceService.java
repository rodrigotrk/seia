...
public class FceService {
	...
	public List<Fce> listarFcePor(AnaliseTecnica analiseTecnica, Boolean indAprovado) {
	    ...
	    // Subquery para filtrar registros
	    DetachedCriteria subquery = DetachedCriteria.forClass(Processo.class, "pro")
	        .createAlias("analiseTecnicaCollection", "a", JoinType.INNER_JOIN)
	        .createAlias("ideRequerimento", "req", JoinType.INNER_JOIN)
	        .setProjection(Projections.property("pro.ideProcesso")) // Retorna apenas ideProcesso
	        .add(Restrictions.eq("a.ideAnaliseTecnica", analiseTecnica.getIdeAnaliseTecnica()));

	    // Consulta principal que consome os resultados da subquery
	    DetachedCriteria criteria = DetachedCriteria.forClass(Fce.class)
	        .createAlias("ideRequerimento", "req", JoinType.INNER_JOIN)
	        .createAlias("ideDocumentoObrigatorio", "doc", JoinType.INNER_JOIN)
	        .createAlias("ideDadoOrigem", "orig", JoinType.INNER_JOIN)
	        .createAlias("ideAnaliseTecnica", "a", JoinType.INNER_JOIN)
	        .createAlias("idePessoaFisica", "func", JoinType.LEFT_OUTER_JOIN)
	        .createAlias("ideProcessoReenquadramento", "pre", JoinType.LEFT_OUTER_JOIN)

	        // Filtros e joins adicionais
	        .add(Subqueries.propertyIn("a.ideProcesso", subquery)) // Filtro com a subquery usando ideProcesso
	    ...
	    criteria.setResultTransformer(new AliasToNestedBeanResultTransformer(Fce.class));
