...
public class PessoaFisicaController extends PessoaEndereco {
	...
	private Boolean disabledProcurador = Boolean.FALSE;
	...
	public void validacaoUsuario() {
	    // 1) Se for um usuário externo, sem pessoa selecionada e ainda não redirecionamos
	    if (isUserExterno()
	            && Util.isNullOuVazio(pessoaSelecionada)
	            && this.refresh == 0) {
	        try {
	            // 1.1) manda pra página /home.xhtml
	            FacesContext.getCurrentInstance()
	                    .getExternalContext()
	                    .redirect("/home.xhtml");
	            // 1.2) garante que não vai redirecionar de novo
	            this.refresh++;
	        } catch (Exception exception) {
	            // 1.3) se der erro no redirect, registra no log e mostra mensagem
	            Log4jUtil.log(this.getClass().getName(), Level.ERROR, exception);
	            JsfUtil.addErrorMessage(exception.getMessage());
	        }
	    }

	    // 2) Se for externo e não escolheu ninguém, bloqueia todo o formulário
	    if (isUserExterno() && Util.isNullOuVazio(pessoaSelecionada)) {
	        this.disableAbasDependencias = true;
	        this.disableFormPessoaFisica = true;
	    }

	    // Inicializa a flag disabledProcurador como FALSE (desabilitado) por padrão
	    this.disabledProcurador = Boolean.FALSE;
	    
	    // Verificar se a pessoa já está salva no banco de dados (tem ID)
	    boolean pessoaJaExiste = !Util.isNullOuVazio(pessoaSelecionada.getId());
	    ...
	    boolean isProprioUsuario = pessoaSelecionada.equals(usuarioLogado);
	    boolean isProcurador = isProcuradorPessoaFisica();
	    
	    if (pessoaJaExiste) {
	        // Pessoa já existe
	        if (isProprioUsuario) {
	            // É o próprio usuário - SEMPRE pode manipular procuradores
	            this.disabledProcurador = Boolean.TRUE;
	        } else if (isProcurador) {
	            // É um procurador - verificar se é o primeiro cadastro
	            // Verificar se já tem procuradores cadastrados
	            ProcuradorPessoaFisica filtro = new ProcuradorPessoaFisica();
	            filtro.setIdePessoaFisica(pessoaSelecionada);
	            Collection<ProcuradorPessoaFisica> procuradores = procuradorPessoaFisicaService.listarProcuradorPessoaFisica(filtro);
	            
	            if (procuradores == null || procuradores.isEmpty()) {
	                // É o primeiro cadastro, procurador pode adicionar a si mesmo
	                this.disabledProcurador = Boolean.TRUE;
	            } else {
	                // Já passou do primeiro cadastro, procurador NÃO pode manipular
	                this.disabledProcurador = Boolean.FALSE;
	            }
	        } else {
	            // Outro tipo de usuário, não permite manipulação
	            this.disabledProcurador = Boolean.FALSE;
	        }
	    } else {
	        // Pessoa ainda não existe (cadastro inicial)
	        // Qualquer usuário pode adicionar procurador no primeiro cadastro
	        this.disabledProcurador = Boolean.TRUE;
	    }

	    // 3) Se o formulário NÃO está habilitado ou foi marcado pra bloquear…
	    if (!this.enableFormPessoaFisica || this.disableFormPessoaFisica) {
	        // 3.1) libera nome e "próprio usuário", mas bloqueia perfil e e-mail
	        ...
	        // 4) Formulário está habilitado, faz checagens mais finas

	        // 4.1) se o usuário já tem um requerimento ou é coordenador, bloqueia o nome
	        if (isExisteRequerimentoPorPF() || isCoordenadorAtendSelicComPermissao()) {
	            this.disabledNome = Boolean.TRUE;
	        }

	        // 4.2) se a pessoa selecionada for o próprio usuário logado…
	        if (pessoaSelecionada.equals(
	                ContextoUtil.getContexto().getUsuarioLogado().getPessoaFisica())) {
	            this.disabledCadastro = Boolean.TRUE;
	            this.disabledProprioUsuario = Boolean.TRUE;
	        } 
	        // 4.3) senão, se for procurador de pessoa física, bloqueia só cadastro
	        else if (isProcuradorPessoaFisica()) {
	            this.disabledCadastro = Boolean.TRUE;
	        }

	        // 4.4) se o perfil estiver bloqueado, bloqueia perfil
	        if (isPerfilBloqueado()) {
	            this.disabledPerfil = Boolean.TRUE;
	        }

	        // 4.5) se o e-mail estiver bloqueado por regra, bloqueia e-mail
	        if (isEmailBloqueado()) {
	            this.disabledEmail = Boolean.TRUE;
	        }
	    }
	...
	public Boolean getDisabledProcurador() {
	    return disabledProcurador;
	}

	public void setDisabledProcurador(Boolean disabledProcurador) {
	    this.disabledProcurador = disabledProcurador;
	}
	...
	private boolean isPrimeiroCadastro() {
	    // Verifica se é um cadastro novo (sem ID) ou se não tem procuradores cadastrados
	    if (Util.isNullOuVazio(pessoaSelecionada.getId())) {
	        return true;  // Se não tem ID, é o primeiro cadastro
	    }
	    
	    // Se já tem ID, verifica se tem procuradores cadastrados
	    return !existemProcuradoresCadastrados();
	}
	...
	private void tratarPessoaSelecionada() {
	    ...
	    if (Util.isNullOuVazio(pessoaSelecionada)) {
	        // Pessoa nova - primeiro cadastro
	        pessoaSelecionada = new PessoaFisica();
	        setPessoa(new Pessoa());
	        
	        // NÃO carregar dados das abas para primeiro cadastro
	        // Inicializar com listas vazias
	        modelTelefone = new ListDataModel<Telefone>(new ArrayList<Telefone>());
	        modelDocumentosIdentificacao = new ListDataModel<DocumentoIdentificacao>(new ArrayList<DocumentoIdentificacao>());
	        listaTelefone = new ArrayList<Telefone>();
	        
	    } else {
	        // Pessoa existente - edição
	        pessoaPesquisa = new PessoaFisica();
	        ...
	        // SÓ carregar dados se a pessoa tem ID (já foi salva)
	        if (!Util.isNullOuVazio(pessoaSelecionada.getId())) {
	            carregarListaTelefone();
	            carregarModelDocumentoIdentificacao();
	        } else {
	            // Mesmo que tenha pessoaSelecionada, se não tem ID, inicializar vazio
	            modelTelefone = new ListDataModel<Telefone>(new ArrayList<Telefone>());
	            modelDocumentosIdentificacao = new ListDataModel<DocumentoIdentificacao>(new ArrayList<DocumentoIdentificacao>());
	            listaTelefone = new ArrayList<Telefone>();
	        }
	...
	public void salvarPessoa() {
	    ...
	    boolean podeSalvar = validarSalvarPessoa();

	    if (podeSalvar) {
	        try {
	            // 1) Mensagem padrão de alteração
	            ...
	            // 4) Busca no banco
	            PessoaFisica resultadoBusca =
	                pessoaFisicaService.buscarPessoaFisicaByCPF(pessoaPesquisa);

	            // 5) Se não existe, faz inclusão imediata
	            if (resultadoBusca == null) {
	                pessoaFisicaService.salvarOuAtualizarPessoaFisica(pessoaSelecionada);
	                pessoaFisicaHistoricoService
	                    .salvarHistoricoPessoaFisica(preencherHistoricoNovaPessoaFisica());
	                disableAbasDependencias = Boolean.FALSE;
	                JsfUtil.addSuccessMessage(msgSucesso);
	                return;
	            }

	            // 6) Se existe, atualiza fluxo de alteração
	            pessoaPesquisa = resultadoBusca;

	            if (pessoaPesquisa.getPessoa().getDesEmail() == null) {
	                pessoaPesquisa.getPessoa().setDesEmail("");
	            }

	            boolean nomeMudou = !pessoaPesquisa.getNomPessoa()
	                                   .equals(pessoaSelecionada.getNomPessoa());
	            boolean emailMudou = !pessoaPesquisa.getPessoa().getDesEmail()
	                                   .equals(pessoaSelecionada.getPessoa().getDesEmail());

	            if (nomeMudou || emailMudou) {
	                pessoaFisicaService.salvarOuAtualizarPessoaFisica(pessoaSelecionada);
	                pessoaFisicaHistoricoService
	                    .salvarHistoricoPessoaFisica(preencherHistoricoPessoaFisica());
	            } else {
	                boolean hasFisica = !Util.isNullOuVazio(pessoaSelecionada.getIdePessoaFisica());
	                Object histAnt = pessoaFisicaHistoricoService
	                                   .buscarHistoricoAnterior(pessoaSelecionada);
	                boolean hasHistAnt = !Util.isNullOuVazio(histAnt);

	                if (hasFisica && hasHistAnt) {
	                    pessoaFisicaService.salvarOuAtualizarPessoaFisica(pessoaSelecionada);
	                } else {
	                    pessoaFisicaService.salvarOuAtualizarPessoaFisica(pessoaSelecionada);
	                    pessoaFisicaHistoricoService
	...
	private void carregarListaTelefone() {
	    ...
	    try {
	        ...
	        if (isPessoaJaSalva()) {
	            // Pessoa já foi salva, pode carregar telefones
	            listaTelefone = telefoneService.buscarTelefonesPorPessoa(getPessoa());
	            modelTelefone = Util.castListToDataModel(listaTelefone);
	        } else {
	            // Pessoa ainda não foi salva, inicializar com lista vazia
	            modelTelefone = new ListDataModel<Telefone>(new ArrayList<Telefone>());
	        }
	    } catch (Exception e) {
	        erro = e;
	...
	/**
	 * Verifica se a pessoa já foi salva no banco (tem ID)
	 * Usado para determinar se deve carregar documentos/telefones
	 */
	private boolean isPessoaJaSalva() {
	    return getPessoa() != null && !Util.isNullOuVazio(getPessoa().getIdePessoa());
	}
	
	public void carregarModelDocumentoIdentificacao() {
	    try {
	        if (isPessoaJaSalva()) {
	            // Pessoa já foi salva, pode carregar documentos
	            this.modelDocumentosIdentificacao = new ListDataModel<DocumentoIdentificacao>(
	                documentoIdenficacaoService.listarDocumentosIdentificacaoPorPessoa(getPessoa())
	            );
	        } else {
	            // Pessoa ainda não foi salva, inicializar com lista vazia
	            this.modelDocumentosIdentificacao = new ListDataModel<DocumentoIdentificacao>(
	                new ArrayList<DocumentoIdentificacao>()
	            );
	        }
	    } catch (Exception e) {
	        Log4jUtil.log(this.getClass().getName(), Level.ERROR, e);
