...
@Named("declaracaoEnderecoGeradorResiduoController")
@ViewScoped
public class DeclaracaoEnderecoGeradorResiduoController {
	...
	@EJB
	ServicoDeCepService cepService;
	...
	public void limparEndereco() {
		...
		this.enderecoSelecionado.setIndPossuiLicencaAutorizacao(Boolean.FALSE);
	...
	public void filtrarPorCep() {
		try {
			...
			if(Util.isNullOuVazio(cepAnterior) || (!cepAnterior.equals(logradouroPesquisa.getNumCep())) || 
					(cepAnterior.equals(logradouroPesquisa.getNumCep()) && 	!logradouroPesquisa.getIdeMunicipio().getIdeMunicipio().equals(MunicipioEnum.SALVADOR.getId()))){
				cepAnterior = logradouroPesquisa.getNumCep();
				if (!Util.isNullOuVazio(logradouroPesquisa.getNumCep())) {
					montaListaBairros();
					if(Util.isNullOuVazio(listaBairro) || (Util.isNull(isEstadoBahia) || (!Util.isNull(isEstadoBahia) && !isEstadoBahia))) {
						...
						municipio = new Municipio();
						enableFormEndereco = false;						
						Html.atualizar("formEnderecoPesquisaCep");
						return;
					}
					logradouro = new Logradouro(0);
					enableFormEndereco = true;
					...
					if(cepService.getTipoLogradouroEnt()!=null) {
						tipoLogradouro = cepService.getTipoLogradouroEnt();
					}						
		...
		try {
			...
			if (!listaBairro.isEmpty() && !Util.isNullOuVazio(listaBairro.get(0))) {
				...
				if(!Util.isNullOuVazio(bairro) && !Util.isNullOuVazio(bairro.getIdeMunicipio()) && !Util.isNullOuVazio(bairro.getIdeMunicipio().getIdeMunicipio())){
					...
					isEstadoBahia = (bairro.getIdeMunicipio().getIdeEstado().getIdeEstado().equals(Integer.valueOf(EstadoEnum.BAHIA.getId())));					
				}else{					
					isEstadoBahia = null;
				}
				showUfCidade = Boolean.FALSE;
	...
	public void visualizarEnderecoGeradorResiduo(DeclaracaoTransporteResiduoEndereco declaracaoEndereco) {
		...
		if(!Util.isNull(declaracaoEndereco) && !Util.isNull(declaracaoEndereco.getEndereco())) {
			try {
				...
				JsfUtil.addErrorMessage("Não foi possível carregar os dados do endereço do gerador de resíduo.");
			}
			...
			Html.atualizar("formEndereco:pnlEndereco");
			Html.atualizar("formEnderecoPesquisaCep");
		}
	}
	...
	public Integer getCepAnterior() {
		return cepAnterior;
	}

	public void setCepAnterior(Integer cepAnterior) {
	...
	public void setListaTipoLogradouroSelectItem(
			Collection<SelectItem> listaTipoLogradouroSelectItem) {
