...
public class EmpreendimentoController extends SeiaControllerAb implements Serializable {
		...
		try {
			...
			if (!Util.isNullOuVazio(empreendimento)) {
				...
				if(!Util.isNullOuVazio(empreendimento.getImovelCollection())){
					...
					abaImoveis = tipoImovel.getIdeTipoImovel();
				}
	...
	public void salvarAtualizarEmpreendimento() {
		...
		try {
			...
			if (Util.isNull(getEmpreendimento().getIdeEmpreendimento())) {
				...
				//Imovel Rural
				if(getAbaImoveis().equals(Integer.valueOf(1))) {
					getImovel().setImovelRural(null);
					getEmpreendimento().setImovelCollection(new ArrayList<Imovel>());
					getImovel().setIdeTipoImovel(new TipoImovel(TipoImovelEnum.RURAL.getId()));
					getImovel().setDtcCriacao(new Date());
					getEmpreendimento().getImovelCollection().add(getImovel());
				}
				//Cessionário
	...
	public void changeListenerTipoImovel(ValueChangeEvent pValueChangeEvent) {
		...
		if (pValueChangeEvent.getNewValue() instanceof Integer) {
			...
			if (vlo == 1 || (!Util.isNullOuVazio(empreendimento.getEnderecoEmpreendimentoCollection())
					&& empreendimento.getEnderecoEmpreendimentoCollection().size() == 2)) {
				
				setAbaImoveis(1);
				desabilitaAbaImovel(false);
				
				TipoImovel tipoRural = new TipoImovel();
				tipoRural.setIdeTipoImovel(TipoImovelEnum.RURAL.getId());
				
				this.imovel.setIdeTipoImovel(tipoRural);
				this.imovel.setImovelRural(new ImovelRural());
				this.imovel.setImovelUrbano(null);
				
			} else { 
				setAbaImoveis(0);
				desabilitaAbaImovel(true);
				
				TipoImovel tipoUrbano = new TipoImovel();
				tipoUrbano.setIdeTipoImovel(TipoImovelEnum.URBANO.getId());
				
				this.imovel.setIdeTipoImovel(tipoUrbano);
				this.imovel.setImovelUrbano(new ImovelUrbano());
				this.imovel.setImovelRural(null);
			}
		}
	}
	...
	public void validarImoveis(Empreendimento empreendimento) {
	    if (empreendimento == null || Util.isNullOuVazio(empreendimento.getImovelCollection())) {
	        return;
	    }

	    List<Imovel> collectionImovel = new ArrayList<Imovel>();

	    for (Imovel imovel : empreendimento.getImovelCollection()) {
	        if (isImovelUrbano(imovel)) {
	            ImovelUrbano imovelUrbano = imovel.getImovelUrbano(); 
	            if (imovelUrbano != null) {
	                imovelUrbano.setIndExcluido(false);
	            }
	        }

	        collectionImovel.add(imovel);
	    }

	    empreendimento.setImovelCollection(collectionImovel);
	}

	private boolean isImovelUrbano(Imovel imovel) {
	    if (imovel == null || imovel.getIdeTipoImovel() == null) {
	        return false; 
	    }
	    return TipoImovelEnum.URBANO.getId().equals(imovel.getIdeTipoImovel().getIdeTipoImovel());
	}
	...
	public boolean isPossuiImovelVinculado() {
		if(!Util.isNullOuVazio(empreendimento) && !Util.isNullOuVazio(empreendimento.getIdeEmpreendimento())) {
			return !Util.isNullOuVazio(empreendimentoService.buscarVinculoImovelEmpreendimento(empreendimento));
		} else {
			return false;
		}
